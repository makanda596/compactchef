...
const Services = () => {
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <button
                className="bg-gold text-gray-900 px-4 py-2 m-4 rounded md:hidden self-start"
                onClick={() => setSidebarOpen(!sidebarOpen)}
            >
                {sidebarOpen ? "Close Services" : "Show Services"}
            </button>

            {/* Sidebar */}
            <aside
                className={`fixed top-0 left-0 h-full bg-gray-800 text-white p-6 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
                    ...
                    {services.map((service) => (
                        <li
                            ...
                            onClick={() => {
                                ...
                                setSidebarOpen(false); // Close the sidebar after selection on small screens
                            }}
                        >
                            {service.title}
                        </li>
